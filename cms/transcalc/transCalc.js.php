...
<script>
	...
	$('#FromName').on('click keyup', function(event) {
        ...
        ajax_request = $.ajax({
            url:  WEBSITE + 'getFromPlacesEdge.php',
            type: 'GET',
            dataType: 'jsonp',
            data: { 
                qry : query
    ...
    $('#ToName').on('click keyup', function(event) {
        ...
        var html = '';		
		var fID=$("#FromID").val();
		var fName=$("#FromName").val();
		var Long=$("#FromLongitude").val();
		var Latt=$("#FromLatitude").val();
		var Country=$("#FromCountry").val();
		if(typeof ajax_request !== 'undefined') ajax_request.abort();
		$.ajax({
			url: WEBSITE + 'getTerminal.php', 
			type: 'GET',
			dataType: 'jsonp',
			async: false,
			data: {Name: fName, Long: Long, Latt: Latt, Country: Country},
			error: function() {},
			success: function(res) {
				window.TerminalID=res.TerminalID; 
				window.TerminalName=res.TerminalName;
				window.TerminalLatitude=res.Latitude;
				window.TerminalLongitude=res.Longitude;
			}
		});	
        query = $("#ToName").val();			
		if (fID==0)	{
			fID=window.TerminalID;
			if (query.length < 3) {
			html +='<a class="list-group-item toName black-text" id="'+ window.TerminalID +
					'" data-name="'+window.TerminalName+'">'+window.TerminalName +'</a>';
			}		
		}			
        ...
        ajax_request = $.ajax({
            ...
            data: {
				...
				tLatitude: window.TerminalLatitude,
				tLongitude: window.TerminalLongitude,
                qry : query
            },
            error: function() {
                //callback();
    ...
    $("#selectCarAdminBtn").click(function(){
		/*if (($("#FromID").val()==0 || $("#ToID").val()==0) && $("#Distance").val()==0) {
			$("#dist").show(500);
		}*/
		//else {	
    ...
    function selectCarAdm(showAlert,ver) {
        ...
        if(proceed) {
			var url = WEBPATH+"/cms/transcalc/selectCarTC.php"; 
