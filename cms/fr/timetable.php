...
?>
	...
	<form action="index.php?p=timetable" method="post" style="display:inline-block;width:50%;">
		<?
		...
		hiddenField('OwnerID', $_SESSION['OwnerID']);
		?>
		<?= SORT ?>:
		<div class="btn-group">
    ...
    <form action="p/modules/timetable/print.php" method="post" target="_blank" style="display:inline-block;width:49%;text-align:right">
		<?
		hiddenField('DateFrom', $_REQUEST['DateFrom']);
		hiddenField('DateTo', $_REQUEST['DateTo']);
		hiddenField('SubDriverID', $_REQUEST['SubDriverID']);
	...
	?>    
		...
		style="cursor:default; padding:8px !important;background:<?= $bgColor ?>">
		    <div class="col-md-2">
		        ...
		        <?
		            if($od->getUserLevelID() == '2') echo " <i class='fa fa-user-secret'></i> ";
		            if($od->getPayNow() > "0") echo " <i class='fa fa-credit-card'></i>";
		        ?>
		        <br>
				<div class="small">
		...
		<div class="row grey lighten-4 pad1em shadow" id="show<?= $i ?>" style="display:none;margin:0">
			...
			<div class="row">
				...
				<div class="col-md-3">
					...
					xonchange="updateNotes('<?= $i ?>');"><?= stripslashes( $od->getSubDriverNote() ) ?></textarea>
				</div>
...
<script type="text/javascript">
	function updateNotes (i)
	{
		...
		$.ajax({
			url: "p/modules/timetable/ajax_updateNotes.php",
			type: "POST",
			data: {
				ID: id,
				OrderID: oid,
