...
<script type="text/javascript">
	...
	var data_v4_SpecialTimes = [];
	function all_v4_SpecialTimes() {
		...
		var callFunction = 'all_v4_SpecialTimesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_SpecialTimes/v4_SpecialTimes_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SpecialTimesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SpecialTimesData.data;
			  recordsTotal = v4_SpecialTimesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_SpecialTimes = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SpecialTimesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_SpecialTimes : data_v4_SpecialTimes});

				$("#show_v4_SpecialTimes").html(HTML);
		  },
	...
	function one_v4_SpecialTimes(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SpecialTimesWrapper"+id).css('display') != 'none') {$("#v4_SpecialTimesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_SpecialTimes/v4_SpecialTimes_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SpecialTimesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_SpecialTimes"+id).html(HTML);

				$("#v4_SpecialTimesWrapper"+id).show('slow');
			},
	...
	function new_v4_SpecialTimes() { 
		var data = {
				...
				$("#new_v4_SpecialTimes").html(HTML);

				$("#v4_SpecialTimesWrapperNew").show('slow');

	}


	function editClosev4_SpecialTimes(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_SpecialTimes(id, inList) { 
	
		if($("#v4_SpecialTimesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SpecialTimesEditForm"+id).serializeObject();
		var formData = $("#v4_SpecialTimesEditForm"+id).serialize();

		// update data on server
		var url = window.root + '/cms/p/modules/v4_SpecialTimes/'+
		'v4_SpecialTimes_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SpecialTimesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_SpecialTimes(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_SpecialTimes/'+
		'v4_SpecialTimes_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_SpecialTimes(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SpecialTimesData(id, newData) {

	  var result = $.grep(data_v4_SpecialTimes, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_SpecialTimes(id, ress);

		    data = data_v4_SpecialTimes;

		    var source   = $("#v4_SpecialTimesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_SpecialTimes : data});
	...
	function changeall_v4_SpecialTimes( id, sve ) {
	   for (var i in data_v4_SpecialTimes) {
		 if (data_v4_SpecialTimes[i].ID == id) {
		    data_v4_SpecialTimes[i] = sve;
