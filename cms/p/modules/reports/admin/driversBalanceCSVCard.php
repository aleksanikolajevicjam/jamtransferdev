<?
    // kopija driversBalanceCSVCash, samo za online (card) umjesto cash
	session_start();
...
if (isset($_REQUEST['StartDate']) and isset($_REQUEST['EndDate']))
{
	...
	$q .= " AND TNo = '1' ";
	//$q .= " AND PayLater != 0 "; // samo cash transferi
	...
	echo '<h2>'.DRIVERS_BALANCE .' - ' . CARD . $_REQUEST['StartDate'] . ' - ' . $_REQUEST['EndDate'] . '</h2><br>';
	...
	while( $o = $e->fetch_object() )
	{
        ...
        echo '</td><td style="vertical-align:top;white-space: nowrap;">';
			...
			echo ' Card: ' . number_format($o->PayLater,2) . ' EUR<br/>';
    ...
    echo ' | Total Card: ' . number_format($totCash, 2);
	...
	echo '<div align="left" >';
	echo '<form action="index.php?p=driversWTransfersCash&StartDate='.$_REQUEST['StartDate'].'&EndDate='.$_REQUEST['EndDate'].'&Submit=1" method="POST">';
	echo '<br/>';
...
else
{
	...
	echo '<form action="index.php?p=driversBalanceCash" method="POST">';
	
	echo '<h1>'.DRIVERS_BALANCE.' - CASH</h1><br>';
