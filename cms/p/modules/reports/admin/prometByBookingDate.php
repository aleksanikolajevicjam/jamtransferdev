...
<form action="index.php?p=promet2" method="post" class="form">
	<div class="row">
		<div class="col-md-2">
			<?= FROM ?> :
		</div>
...
$order_price = $o->DetailPrice+$o->ExtraCharge;
	...
	*/
    ...
    $netto = $order_price - $o->ProvisionAmount - $cijena_vozaca;
    $netto_sum += $netto;
	...
	elseif($o->UserLevelID == 41) { $rowClass="class='danger'"; $operator += 1; $operatorTotal += $order_price;}
	elseif($o->UserLevelID == 91) { $rowClass="class='danger'"; $admin += 1; $adminTotal += $order_price;}
	else $rowClass = '';
...
?>    
  ...
  <p class="alert alert-danger">
  	<?= $operator ?> <?= OPERATOR_ORDERS ?> : <?= number_format($operatorTotal,2,',','.'); ?>
  </p>
...
<div class="container lead">
	...
	<div class="row">
		...
		<div class="col-md-2" align="right">
			<? echo number_format($sum_price - $trosak_vozaca - $provision_sum,2,',','.'); ?>
		</div>
	</div>
