...
if( isset($_REQUEST['submit']) or isset($_REQUEST['Save']) ) {
    ...
    for($day = 1; $day <= $daysInMonth; $day++) {
        ...
        $where  = "WHERE SubPickupDate = '" . $dateFrom . "' ";
        $where .= "AND DriverID = '". $DriverID ."' ";
        $where .= "AND TransferStatus != '3' ";
        $where .= "AND TransferStatus != '9' ";
        $where .= "AND TransferStatus != '4' ";
        ...
        if($ShowHidden)     $odk = $od->getKeysBy("SubPickupDate", "ASC, SubPickupTime ASC", $where);
        else                $odk = $od->getKeysBy("SubPickupDate", "ASC, SubPickupTime ASC", $where.$where2);

        if($ShowHidden)     $odk2 = $od->getKeysBy("SubPickupDate", "ASC, SubPickupTime ASC",  $where.$where2);
        ...
        if(true) {        
            ...
            foreach( $odk as $nn => $DetailsID ) {
                ...
                if($od->SubPickupDate == $lastDate) {
                
                    if($od->SubPickupDate != NULL and $expectedArrival != NULL and $od->SubPickupTime != NULL) {
                        $start_date = new DateTime($dateFrom . " ". $expectedArrival.":00");
                        $since_start = $start_date->diff(new DateTime($dateFrom . " " . $od->SubPickupTime .":00"));                
                ...
                if( !empty($od->RouteID) ) {
					...
					// 23.07. Mandic zatrazio da se datum transfera prikaziva PickupDate (bio SubPickupDate), vrijeme treba ostati SubPickupTime - Promjenio Leo
