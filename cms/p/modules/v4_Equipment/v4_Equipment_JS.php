...
<script type="text/javascript">
	...
	var data_v4_Equipment = [];
	function all_v4_Equipment() {
		...
		var callFunction = 'all_v4_EquipmentFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_Equipment/v4_Equipment_All.php?where='+where+
...
console.log (url);
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_EquipmentData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_EquipmentData.data;
			  recordsTotal = v4_EquipmentData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Equipment = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_EquipmentListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Equipment : data_v4_Equipment});

				$("#show_v4_Equipment").html(HTML);
		  },
	...
	function one_v4_Equipment(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_EquipmentWrapper"+id).css('display') != 'none') {$("#v4_EquipmentWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_Equipment/v4_Equipment_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_EquipmentEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Equipment"+id).html(HTML);

				$("#v4_EquipmentWrapper"+id).show('slow');
			},
	...
	function new_v4_Equipment() { 
		var data = {
				...
				$("#new_v4_Equipment").html(HTML);

				$("#v4_EquipmentWrapperNew").show('slow');

	}


	function editClosev4_Equipment(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Equipment(id, inList) {  
		if($("#v4_EquipmentEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_EquipmentEditForm"+id).serializeObject();
		var formData = $("#v4_EquipmentEditForm"+id).serialize();
console.log( window.root + '/cms/p/modules/v4_Equipment/'+
		'v4_Equipment_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData)
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Equipment/'+
		'v4_Equipment_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		$.ajax({
			type: 'POST',
			url: url,
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_EquipmentData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Equipment(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Equipment/'+
		'v4_Equipment_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Equipment(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_EquipmentData(id, newData) {

	  var result = $.grep(data_v4_Equipment, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Equipment(id, ress);

		    data = data_v4_Equipment;

		    var source   = $("#v4_EquipmentListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Equipment : data});
	...
	function changeall_v4_Equipment( id, sve ) {
	   for (var i in data_v4_Equipment) {
		 if (data_v4_Equipment[i].ID == id) {
		    data_v4_Equipment[i] = sve;
