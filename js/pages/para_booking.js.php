...
function selectFrom(selected) {
		...
		ReplaceSelectorText("#toSelector", ' ---');
		$("#select2-toSelector-container").html('---');
...
// CAR PANEL CLICKED
	function carSelected(linkId) {
		...
		*/
		...
		var vehicledesc 	= $("#v"+linkId).attr("data-vehicledesc");
		...
		$("#VehicleDescription").val(vehicledesc);
		$("#Price").val(price);
...
// VEHICLE NUMBER CHANGED
    function vehicleNumber(basePrice,id) {
        ...
        $("#priceDisplay"+id).html(parseFloat(newPrice).toFixed(2));
    }	
