<?	
	// priprema podataka
	require_once $_SERVER['DOCUMENT_ROOT']. '/api/getExtras.php';
...
if(!empty($extras)) { ?>
	...
	<div class="row" style="margin:0px !important">
		<div class="col s12 l7 black-text">
			<?=  $Service ?> (<?= nf(toCurrency($Price)*s('ExchFaktor')) . ' ' . s('Currency') ?>)
		</div>
		...
		<div class="col s11 l2">
			...
			onchange="recalcExtra(this,'<?= $Price*s('ExchFaktor') ?>');">
...
?>
		...
		<div class="row blue xdarken-3 white-text xl z-depth-2 center">
				...
				<div  class="col s12 "><?= GRAND_TOTAL ?>:
					...
					<span class="grandTotal"><?= nf(toCurrency(s('Price'))) ?> <?= s('Currency') ?></span>
					<? if(s('Provision')>0) {?><span> - <?= nf(toCurrency($disprice)) ?> <?= s('Currency') ?> (<?= PROVISION ?>)</span><? } ?>
				</div>

		</div>
<?
// javascript je u js/pages/final.js.php !!!
