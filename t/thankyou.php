...
@Blogit($_REQUEST);    
	...
	if($_SESSION['PaymentOption'] != '2' and $_SESSION['PaymentOption'] != '4') { // 2= cash only
		# DATA FROM WEBTEH
	...
	else if ($_SESSION['PaymentOption'] == '2' or $_SESSION['PaymentOption'] == '4') {
		 $response_code = '0000';
	}
	...
	if(!$_SESSION['REFRESHED']) {
		...
		if($response_code == '0000') {
		    // Upis u bazu
		    $OrderKey = '';
		    require_once $_SERVER['DOCUMENT_ROOT'] . '/cms/t/dbAddOrder2.php';
		...
		<?
			...
			if( count($oXkey) > 0 ){
				...
				foreach($oXkey as $i => $id) {
					...
					echo '<td>' . 
						//Eur2( $ox->getSum(),$om->getMOrderCurrency() ) . ' ' . $om->getMOrderCurrency() . //ne prikazivati cenu
					'</td></tr>';
