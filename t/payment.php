...
require_once $_SERVER['DOCUMENT_ROOT'] . '/t/dbAddOrder.php';
...
@Blogit('AGENT ONLINE - kreiranje amounta');
	...
	$AgentCommision = nf($_POST['TT'] * $user['Provision'] / 100);
...
@Blogit('AGENT ONLINE - webteh loop');
	...
	<form id="webteh" action="<?=$submit_url?>/form" method="post">
		...
		<input id="ch_full_name" name="ch_full_name" type="hidden" value="" />
		<input id="ch_address" name="ch_address" type="hidden" value="" />
		<input id="ch_city" name="ch_city" type="hidden" value="" />
		<input id="ch_zip" name="ch_zip" type="hidden" value="" />
		<input id="ch_country" name="ch_country" type="hidden" value="" />
		<input id="ch_phone" name="ch_phone" type="hidden" value="" />
		<input id="ch_email" name="ch_email" type="hidden" value="" />
	...
	if ($insertSuccess) { ?>
		<form id="thx" action="index.php?p=agentThankyou" method="post"></form>
