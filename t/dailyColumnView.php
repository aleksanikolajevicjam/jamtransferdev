	...
	foreach ($subDArray as $SubDriver) { // STUPAC (driver)
		...
		foreach ($row as $rw) {
			if ($rw->SubDriver==$SubDriver || $rw->SubDriver2==$SubDriver || $rw->SubDriver3==$SubDriver) 
				...
				$q = "SELECT * FROM `v4_UserLocations` WHERE `UserID`=".$SubDriver." order by time desc"; 
				$r = $db->RunQuery($q);
				$loc=array();
				while ($t = $r->fetch_object()) {
					$loc[] = $t;
		...
		foreach ($DetailsIDArray as $ID) { // REDAK u STUPCU (transfer)
			...
			if ($display_location && $od->getTransferStatus() != "5") {
				?>
					<div>
						<b>Last location at <?= date('h:i:s T',$lc->Time) ?></b>
						<div>
							<iframe src="https://maps.google.com/maps?q=<?= $lc->Lat ?>, <?= $lc->Lng ?>&z=10&output=embed"  frameborder="0" style="border:0"></iframe>
						</div>						
					</div>	
				<?	
...
echo $currenttime;
			...
			else if ($od->getPickupTime()<$currenttime) $bgColor = "#00FF00";
