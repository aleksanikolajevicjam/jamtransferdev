...
Class v4_OrdersMaster {
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_OrdersMasterArh where MOrderID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_OrdersMasterArh WHERE MOrderID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_OrdersMasterArh set 
...
MOrderLang = '".$this->myreal_escape_string($this->MOrderLang)."' WHERE MOrderID = '".$this->MOrderID."'");
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT MOrderID from v4_OrdersMasterArh $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["MOrderID"];
				$i++;
			}
