...
Class v4_SurGlobal {
     ...
     *
     */
	public function New_v4_SurGlobal($SiteID,$OwnerID,$Category,$NightStart,$NightEnd,$NightPercent,$NightAmount,$WeekendPercent,$WeekendAmount,$MonPercent,$MonAmount,$TuePercent,$TueAmount,$WedPercent,$WedAmount,$ThuPercent,$ThuAmount,$FriPercent,$FriAmount,$SatPercent,$SatAmount,$SunPercent,$SunAmount,$S1Start,$S1End,$S1Percent,$S2Start,$S2End,$S2Percent,$S3Start,$S3End,$S3Percent,$S4Start,$S4End,$S4Percent,$S5Start,$S5End,$S5Percent,$S6Start,$S6End,$S6Percent,$S7Start,$S7End,$S7Percent,$S8Start,$S8End,$S8Percent,$S9Start,$S9End,$S9Percent,$S10Start,$S10End,$S10Percent){
		$this->SiteID = $SiteID;
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_SurGlobal where ID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_SurGlobal WHERE ID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_SurGlobal set 
SiteID = '".$this->myreal_escape_string($this->SiteID)."', 
OwnerID = '".$this->myreal_escape_string($this->OwnerID)."', 
Category = '".$this->myreal_escape_string($this->Category)."', 
...
S10Percent = '".$this->myreal_escape_string($this->S10Percent)."' WHERE ID = '".$this->ID."'");
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT ID from v4_SurGlobal $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["ID"];
				$i++;
			}
	...
	public function fieldNames(){
		$fieldNames = array(
			'ID',			'SiteID',			'OwnerID',			'Category',			'NightStart',			'NightEnd',			'NightPercent',			'NightAmount',			'WeekendPercent',			'WeekendAmount',			'MonPercent',			'MonAmount',			'TuePercent',			'TueAmount',			'WedPercent',			'WedAmount',			'ThuPercent',			'ThuAmount',			'FriPercent',			'FriAmount',			'SatPercent',			'SatAmount',			'SunPercent',			'SunAmount',			'S1Start',			'S1End',			'S1Percent',			'S2Start',			'S2End',			'S2Percent',			'S3Start',			'S3End',			'S3Percent',			'S4Start',			'S4End',			'S4Percent',			'S5Start',			'S5End',			'S5Percent',			'S6Start',			'S6End',			'S6Percent',			'S7Start',			'S7End',			'S7Percent',			'S8Start',			'S8End',			'S8Percent',			'S9Start',			'S9End',			'S9Percent',			'S10Start',			'S10End',			'S10Percent'		);
		return $fieldNames;
	}
    /**
     * Close mysql connection
     */
	public function endv4_SurGlobal(){
