...
Class v4_OrderDetailsTempOld {
     ...
     *
     */
	public function New_v4_OrderDetailsTempOld($SiteID,$OrderID,$UserID,$UserLevelID,$AgentID,$CustomerID,$TransferStatus,$OrderDate,$TaxidoComm,$ServiceID,$RouteID,$FlightNo,$FlightTime,$PaxName,$PickupID,$PickupName,$PickupPlace,$PickupAddress,$PickupDate,$PickupTime,$PickupNotes,$DropID,$DropName,$DropPlace,$DropAddress,$DropNotes,$PriceClassID,$DetailPrice,$DriversPrice,$Discount,$DriverExtraCharge,$Provision,$ExtraCharge,$PaymentMethod,$PaymentStatus,$PayNow,$PayLater,$InvoiceAmount,$ProvisionAmount,$PaxNo,$VehiclesNo,$VehicleType,$VehicleID,$DriverID,$DriverName,$DriverEmail,$DriverTel,$DriverConfStatus,$DriverConfDate,$DriverConfTime,$DriverNotes,$DriverPayment,$DriverPaymentAmt,$Rated,$DriverPickupDate,$DriverPickupTime,$SubDriver,$Car,$CashIn,$FinalNote){
		$this->SiteID = $SiteID;
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_OrderDetailsTempOld where DetailsID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_OrderDetailsTempOld WHERE DetailsID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_OrderDetailsTempOld set 
...
FinalNote = '".$this->myreal_escape_string($this->FinalNote)."' WHERE DetailsID = '".$this->DetailsID."'");
	return $result; 
}
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT DetailsID from v4_OrderDetailsTempOld $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
	...
	public function fieldNames(){
		$fieldNames = array(
			'SiteID',			'DetailsID',			'OrderID',			'UserID',			'UserLevelID',			'AgentID',			'CustomerID',			'TransferStatus',			'OrderDate',			'TaxidoComm',			'ServiceID',			'RouteID',			'FlightNo',			'FlightTime',			'PaxName',			'PickupID',			'PickupName',			'PickupPlace',			'PickupAddress',			'PickupDate',			'PickupTime',			'PickupNotes',			'DropID',			'DropName',			'DropPlace',			'DropAddress',			'DropNotes',			'PriceClassID',			'DetailPrice',			'DriversPrice',			'Discount',			'DriverExtraCharge',			'Provision',			'ExtraCharge',			'PaymentMethod',			'PaymentStatus',			'PayNow',			'PayLater',			'InvoiceAmount',			'ProvisionAmount',			'PaxNo',			'VehiclesNo',			'VehicleType',			'VehicleID',			'DriverID',			'DriverName',			'DriverEmail',			'DriverTel',			'DriverConfStatus',			'DriverConfDate',			'DriverConfTime',			'DriverNotes',			'DriverPayment',			'DriverPaymentAmt',			'Rated',			'DriverPickupDate',			'DriverPickupTime',			'SubDriver',			'Car',			'CashIn',			'FinalNote'		);
		return $fieldNames;
	}
    /**
     * Close mysql connection
     */
	public function endv4_OrderDetailsTempOld(){
