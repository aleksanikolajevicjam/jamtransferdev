...
Class v4_DriverRoutes {
     ...
     *
     */
	public function New_v4_DriverRoutes($SiteID,$OwnerID,$RouteID,$FromID,$ToID,$Active,$Approved,$RouteName,$RouteNameEN,$RouteNameRU,$RouteNameFR,$RouteNameDE,$RouteNameIT,$RouteNameSE,$RouteNameNO,$RouteNameES,$RouteNameNL,$OneToTwo,$TwoToOne,$SurCategory,$SurID){
		$this->SiteID = $SiteID;
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_DriverRoutes where ID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_DriverRoutes WHERE ID = $key_row");
	}

    /**
     * Delete the row by using the RouteID as arg
     *
     * @param key_table_type $key_row
     *
     */
	public function deleteRouteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_DriverRoutes WHERE RouteID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_DriverRoutes set 
...
SurID = '".$this->myreal_escape_string($this->SurID)."' WHERE ID = '".$this->ID."'");
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT ID from v4_DriverRoutes $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["ID"];
				$i++;
			}
	...
	public function fieldNames(){
		$fieldNames = array(
			'ID',			'SiteID',			'OwnerID',			'RouteID',			'FromID',			'ToID',			'Active',			'Approved',			'RouteName',			'RouteNameEN',			'RouteNameRU',			'RouteNameFR',			'RouteNameDE',			'RouteNameIT',			'RouteNameSE',			'RouteNameNO',			'RouteNameES',			'RouteNameNL',			'OneToTwo',			'TwoToOne',			'SurCategory',			'SurID'		);
		return $fieldNames;
	}
    /**
     * Close mysql connection
     */
	public function endv4_DriverRoutes(){
