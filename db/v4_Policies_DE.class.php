...
Class v4_Policies_DE {
     ...
     *
     */
	public function New_v4_Policies_DE($DateChanged,$BookingAdvance,$DeclineTime,$CancelDays,$CancelCharge,$Deposit,$AMEX,$Visa,$MasterCard,$Diners,$MeetingGeneral,$MeetingAirport,$MeetingFerry,$MeetingBus,$MeetingTrain,$Locked){
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_Policies_DE where OwnerID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_Policies_DE WHERE OwnerID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_Policies_DE set 
...
Locked = '".$this->myreal_escape_string($this->Locked)."' WHERE OwnerID = '".$this->OwnerID."'");
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT OwnerID from v4_Policies_DE $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["OwnerID"];
				$i++;
			}
     ...
     */
	public function endv4_Policies_DE(){
