...
if (isset($_REQUEST['showPrices'])) $showPrices=true;
else $showPrices=false;
?>

	<div class="container white" id="voucher">
	<? 
	    $html =  printAgentVoucher($OrderID, $showPrices);
...
function printAgentVoucher($OrderID, $showPrices = true) {
	...
	if(count($oKey) == 1) {
		...
		if($om->getMEurToCurrencyRate()>1) $currency='RSD';
		else $currency=$om->getMOrderCurrency();
...
*/?>
		...
		<? if($showPrices == true) { ?>
			<tr>
				...
				<td>
				<strong><?= Eur2(($om->getMTransferPrice()-$om->getMAgentCommision())*$om->getMEurToCurrencyRate(),$om->getMOrderCurrency()) . ' ' . 
									$currency ?></strong>
				</td>
			</tr>
		<? } ?>
		...
		<?
			...
			if( count($oXkey) > 0 ){
				...
				foreach($oXkey as $i => $id) {
					...
					echo '<td>' . 
						Eur2( $ox->getSum()*$om->getMEurToCurrencyRate(),$currency ) . ' ' . $currency . 
					'</td></tr>';
		...
		<? if($showPrices == true) { ?>
			<tr>
				...
				<td>
					<hr>
					<h3>
						<?= nf(($om->getMOrderPriceEUR()-$om->getMAgentCommision()+$om->getMExtrasPrice())*$om->getMEurToCurrencyRate()) . ' ' . $currency ?>
					</h3>
					<hr>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<h3><?= Eur2($om->getMPayLater()*$_SESSION['CurrencyRate'],$om->getMOrderCurrency() ) . ' ' . 
									$_SESSION['Currency2'] ?>
								<? if($_SESSION['Currency2']=='HRK') {?>	
									/ <?= Eur2($od->getPayLater(),$om->getMOrderCurrency() ) . ' ' . 
