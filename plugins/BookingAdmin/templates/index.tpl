<div style="
  ...
  ">
	<br />
	<div class="container pad1em" style="
    ...
    ">
		<div class="row">
			...
			<div class="col s12 xgrey xlighten-3">
				<br />
				<form id="bookingForm" name="bookingForm" action="" method="POST" enctype="multipart/form-data"
					...
					<div class="col l6 s12">
						<label for="AuthUserIDe"><i class="fa fa-globe"></i> Book as
							<strong>Agent</strong></label><br />
						<div>
					...
					<div class="col s12 l2" id="webyblock" style="display: none">
						...
						<select id="wref" class="browser-default" name="wref" value=""></select>
					</div>
					...
					<div class="col l6 s12">
						...
						<div id="selectTo_options" class="list-group white" style="max-height: 15em; overflow: auto">
						</div>
					</div>
					<!-- to -->
					<div class="col l6 s12">
					...
					<div class="col s12 pad1em white-text"
						style="padding: 1rem !important; background: rgba(0, 0, 0, 0.5)">
...
<script>
	...
	$("#wref").on("change", function () {
		...
		if (code != "") {
			...
			var param =
				"code=" + code + "&form=" + "booking" + "&weby_key=" + weby_key;
			$.ajax({
				type: "POST",
				url: link,
				data: param,
				...
				success: function (data) {
					if (data == "false") alert("Wrong reservation reference");
						...
						$("#PickupAddress").val(
							$("#SPAddressHotel").val() + " " + $("#PickupAddress").val()
						);
						$("#DropAddress").val(
							$("#SDAddressHotel").val() + " " + $("#DropAddress").val()
						);
						$("#RPickupAddress").val(
							$("#RPAddressHotel").val() + " " + $("#RPickupAddress").val()
						);
						$("#RDropAddress").val(
							$("#RDAddressHotel").val() + " " + $("#RDropAddress").val()
						);
	...
	$("#srn").on("change", function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if ($("#FlightNo").val() == " ")
					$("#FlightNo").val(
						$("#DFlightCo").val() + " " + $("#DFlightNo").val()
					);
				$("#RFlightNo").val(
					$("#RFlightCo").val() + " " + $("#RFlightNo").val()
				);
				if ($("#RFlightNo").val() == " ")
					$("#RFlightNo").val(
						$("#RDFlightCo").val() + " " + $("#RDFlightNo").val()
					);
				if ($("#FlightTime").val() == "")
					$("#FlightTime").val($("#DFlightTime").val());
				if ($("#RFlightTime").val() == "")
					$("#RFlightTime").val($("#RDFlightTime").val());
			},
		});
	});
</script>